...
program main 
    ...
    ! complex (kind=8), dimension(time_steps) :: 
    ...
    do sim = 1, num_of_simulations
        ...
        do t = 1, size(time_list)
            ...
            if (rand_num <= 2 * modulo_func(g_1_new)**2) then 
                ...
                avg_sigma_L_list(t) = avg_sigma_L_list(t) + (conjg(g_0_new) * e_0_new)
                avg_sigma_R_list(t) = avg_sigma_R_list(t) + (conjg(e_0_new) * g_0_new)

                g2(t) = g2(t) + real(e_0_new * e_0_new)
