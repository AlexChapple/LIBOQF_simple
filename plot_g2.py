...
for Omega in Omega_list:
    ...
    name = directory + "g2_" + (str(Omega).replace(".", "")) + ".txt"

    g2_list.append(np.loadtxt(name))

time_list = g2_list[0][:,0]
...
for Omega in Omega_list:
    ...
    temp_list = [g2(t, Omega) for t in time_list]
...
for index in range(len(Omega_list)):
    ...
    plt.plot(time_list, g2_list[index][:,1], lw=5, c=colour_list[index], label=("$\Omega$ = " + str(Omega_list[index])))

    if index == 2:
        plt.plot(time_list, g2_list_analytical[index], lw=5, c=colours.greek_dark_blue, ls="dotted", label="analytical")
    else:
        plt.plot(time_list, g2_list_analytical[index], lw=5, c=colours.greek_dark_blue, ls="dotted")
