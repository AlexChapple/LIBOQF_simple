...
program main 
    ...
    integer, parameter :: end_time = 13d0 
    integer (kind=8), parameter :: time_steps = 13000d0 
    ...
    do sim = 1, num_of_simulations
        ...
        do t = 1, size(time_list)
            ...
            total = sqrt(modulo_func(g_0)**2 + modulo_func(e_0)**2 &
                    + modulo_func(g_L)**2 + modulo_func(e_L)**2 & 
                    + modulo_func(g_R)**2 + modulo_func(e_R)**2)
            ...
            if (mod(t,period) == 0) then ! Check the boxes now 
                ...
                prob = modulo_func(g_L)**2 + modulo_func(g_R)**2 
                ...
                coeffs(1) = g_0; coeffs(2) = e_0
                coeffs(3) = g_L; coeffs(4) = e_L
                coeffs(5) = g_R; coeffs(6) = e_R
